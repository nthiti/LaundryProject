    ...
    if ($query_user) {
        ...
        if (mysqli_num_rows($query_existing_booking) > 0) {
            echo "<script> alert('วันและเวลาที่คุณเลือกถูกจองไปแล้ว กรุณาเช็ควันและเวลาก่อนทำการจอง ขอบคุณค่ะ'); 
                window.location.href = 'booking10_ad.php'; </script>";
            exit;
        } else {
            $all_washers = array("10.5kg.01", "10.5kg.02", "10.5kg.03", "10.5kg.04");
            ...
            if ($result_check_booked) {
                ...
                header("location: booking10_ad.php");
                exit;
            }
            ...
            if ($query_run) {
                ...
                header("location: payment_ad.php");
            } else {
