...
$ids = $_GET['id'];
$sql = "SELECT * FROM dryer WHERE id ='$ids' ";
...
<div class="h4 text-center alert alert-success mt-5 mb-10 " role="alert">
  แก้ไขข้อมูลเครื่องอบผ้า
</div>
  <form action="update_dryer.php" method="post">
                <label>Id เครื่องอบ</label>
                <input type="number" name="id" class="form-control" readonly value=<?=$row['id']?> > <br>
                <label>น้ำหนักเครื่องอบ</label>
                ...
                <label>ราคา</label>
                    <input type="number" name="price" class="form-control" value=<?=$row['price']?> id ="text/javascript"  ><br>
